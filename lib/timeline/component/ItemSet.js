...
ItemSet.prototype.setSelection = function(ids) {
  var i, ii, id, item, selected, current;
  if (ids == undefined) ids = [];
  if (!Array.isArray(ids)) ids = [ids];
  selected = []; //newly selected list
  current = this.selection||[];

  // unselect items which are currently selected but not present in new selection
  for (i = 0, ii = current.length; i < ii; i++) {
    id = current[i];
    item = this.items[id];
    if(ids.indexOf(id) < 0 && item){
      item.unselect();
    }
  }
